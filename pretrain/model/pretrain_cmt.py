...
class GlocalTextPathCMTPreTraining(BertPreTrainedModel):#BertPreTrainedModel
    ...
    def forward(self, batch, task, compute_loss=True):
        ...
        if task.startswith('mlm'):
            return self.forward_mlm(
                batch['txt_ids'], batch['txt_lens'], batch['txt_embeds'], batch['traj_view_img_fts'], 
    ...
    def forward_mlm(
        ...
        txt_embeds = self.bert.forward_mlm(
            txt_ids, txt_lens, txt_embeds, traj_view_img_fts, traj_obj_img_fts, traj_loc_fts, traj_nav_types, 
            traj_step_lens, traj_vp_view_lens, traj_vp_obj_lens, traj_vpids, traj_cand_vpids,
            gmap_lens, gmap_step_ids, gmap_pos_fts, gmap_pair_dists, gmap_vpids, vp_pos_fts,
        )
    ...
    def forward_sap(
        ...
        batch_size =  txt_ids.size(0) if txt_embeds is None else txt_embeds.size(0)
