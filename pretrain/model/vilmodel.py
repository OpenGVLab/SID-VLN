...
class BertSelfAttention(nn.Module):#
    ...
    def forward(self, hidden_states, attention_mask, head_mask=None):
        ...
        #(batch_size, seq_length, all_head_size) to (batch_size, num_attention_heads, seq_length, attention_head_size)
...
class GraphLXRTXLayer(nn.Module):
    def __init__(self, config):
        ...
        if config.use_lang2visn_attn:#only used in 
...
class ImageEmbeddings(nn.Module):#process image features Rt and object features Ot of current node
    def __init__(self, config):
        super().__init__()
...
class GlobalMapEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.gmap_pos_embeddings = nn.Sequential(#Embedding of the orientation and distance relative to the current node
            nn.Linear(config.angle_feat_size + 3, config.hidden_size),
            BertLayerNorm(config.hidden_size, eps=1e-12)
        )
        #Embedding of the latest visited time step
...
class ImageQueryEmbeddings(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.img_linear = nn.Linear(config.image_feat_size, config.hidden_size)
        self.img_layer_norm = BertLayerNorm(config.hidden_size, eps=1e-12)
    ...
    def forward(
        ...
        target_embeds = self.img_layer_norm(self.img_linear(target_img_fts))
        traj_embeds = self.dropout(target_embeds)
        return traj_embeds
...
class GlocalTextPathCMT(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.img_query_embeddings = ImageQueryEmbeddings(config)
        
        self.init_weights()

    def forward(
        self, txt_ids, txt_lens, txt_embeds, traj_view_img_fts, traj_obj_img_fts, traj_loc_fts, traj_nav_types, 
        ...
        if txt_embeds is None:
            ...
            txt_embeds = self.embeddings(txt_ids, token_type_ids=txt_token_type_ids)#text embedding
            txt_masks = gen_seq_masks(txt_lens)
            txt_embeds = self.lang_encoder(txt_embeds, txt_masks)#multilayer transformer
        else:
            txt_embeds = self.img_query_embeddings(txt_embeds)
            txt_masks = torch.ones(txt_embeds.size(0),1).bool().cuda()
    ...
    def forward_mlm(#only used in model's forward_mlm
        self, txt_ids, txt_lens, txt_embeds, traj_view_img_fts, traj_obj_img_fts, traj_loc_fts, traj_nav_types, 
        traj_step_lens, traj_vp_view_lens, traj_vp_obj_lens, traj_vpids, traj_cand_vpids,
        gmap_lens, gmap_step_ids, gmap_pos_fts, gmap_pair_dists, gmap_vpids, vp_pos_fts,
    ):
