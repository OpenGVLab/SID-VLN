...
from data.dataset import ReverieTextPathData, GoalPathData
...
def main(opts):
        ...
        if opts.init_pretrained == 'bert':
            ...
            for param_name, param in tmp.items():
                ...
                if 'bert.encoder.layer' in param_name:
                    ...
                    checkpoint[param_name] = param
            del tmp
        elif opts.init_pretrained == 'pt':
            checkpoint = torch.load(
                '../datasets/pretrained/model_step_92000.pt', 
                map_location=lambda storage, loc: storage
            )
    ...
    #         data_cfg.train_traj_files, data_cfg.img_ft_files, data_cfg.obj_ft_file,
    ...
    #         max_txt_len=opts.max_txt_len, max_objects=opts.max_objects, in_memory=False, sel_data_idxs = None if opts.world_size < 2 else (opts.local_rank, opts.world_size)
    # )
    # val_nav_db1 = ReverieTextPathData(
    #         data_cfg.val_seen_traj_files, data_cfg.img_ft_files, data_cfg.obj_ft_file,
    #         data_cfg.scanvp_cands_file, data_cfg.connectivity_dir,
    #         image_prob_size=model_config.image_prob_size,
    #         image_feat_size=model_config.image_feat_size, 
    #         angle_feat_size=model_config.angle_feat_size,
    ...
    # )
    # val2_nav_db1 = ReverieTextPathData(
    #         data_cfg.val_unseen_traj_files, data_cfg.img_ft_files, data_cfg.obj_ft_file,
    #         data_cfg.scanvp_cands_file, data_cfg.connectivity_dir,
    #         image_prob_size=model_config.image_prob_size,
    #         image_feat_size=model_config.image_feat_size, 
    #         angle_feat_size=model_config.angle_feat_size,
    ...
    train_nav_db = GoalPathData(
        ...
        obj_prob_size=model_config.obj_prob_size,
        max_txt_len=opts.max_txt_len, max_objects=opts.max_objects, in_memory=True,
        sel_data_idxs = None if opts.world_size < 2 else (opts.local_rank, opts.world_size),
    )
    val_nav_db1 = GoalPathData(
        data_cfg.val_seen_traj_files, data_cfg.img_ft_files, data_cfg.obj_ft_file,
        data_cfg.scanvp_cands_file, data_cfg.connectivity_dir,
        image_prob_size=model_config.image_prob_size,
        image_feat_size=model_config.image_feat_size, 
    ...
    val_dataloaders1 = create_dataloaders(
        data_cfg, val_nav_db1, tokenizer, False, device, opts
    )
    val2_dataloaders1 = create_dataloaders(
        data_cfg, val2_nav_db1, tokenizer, False, device, opts
    )
    ...
    validate(model, val_dataloaders1, setname='_seen')
    LOGGER.info(f'------Step {global_step}: start validation unseen------')
    validate(model, val2_dataloaders1, setname='_unseen')

    for step, (name, batch) in enumerate(meta_loader):
        # forward pass
        if 'txt_embeds' in batch:
            n_examples[name] += batch['txt_embeds'].size(0)
        else:
