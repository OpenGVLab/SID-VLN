...
class MlmDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        output = {}
        if 'txt_embeds' in inputs and not inputs['txt_embeds'] is None:
            output['txt_ids'] = None
            output['txt_labels'] =  torch.LongTensor([1])
            output['txt_embeds'] = inputs['txt_embeds']
        else:
...
def mlm_collate(inputs):
    ...
    if 'txt_embeds' in inputs[0].keys() and not inputs[0]['txt_embeds'] is None:       
        # text batches
        batch['txt_lens'] = None
        batch['txt_ids'] = None
        batch['txt_embeds'] = torch.stack([torch.tensor(embed) for embed in batch['txt_embeds']]).unsqueeze(1)
...
def mrc_collate(inputs):
    ...
    if 'txt_embeds' in inputs[0].keys() and not inputs[0]['txt_embeds'] is None:       
        ...
        batch['txt_ids'] = None
        batch['txt_embeds'] = torch.stack([torch.tensor(embed) for embed in batch['txt_embeds']]).unsqueeze(1)
        # batch['txt_embeds'] = pad_tensors(batch['txt_embeds'])
    else:
        # text batches
        batch['txt_lens'] = torch.LongTensor([len(x) for x in batch['txt_ids']])
        batch['txt_ids'] = pad_sequence(batch['txt_ids'], batch_first=True, padding_value=0)
...
class SapDataset(Dataset):
    def __init__(self, nav_db, tok, end_vp_pos_ratio=0.15):
    ...
    def __getitem__(self, idx):
        ...
        inputs = self.nav_db.get_input(idx, np.random.rand(), return_act_label=True)

        output = {}

        if 'txt_embeds' in inputs and not inputs['txt_embeds'] is None:
            output['txt_ids'] = None
            output['txt_labels'] = None
            output['txt_embeds'] = inputs['txt_embeds']
        else:
            output['txt_ids'] = torch.LongTensor(inputs['instr_encoding'])
        ...
        output['vp_pos_fts'] = torch.from_numpy(inputs['vp_pos_fts'])
        output['vp_angles'] = inputs['vp_angles']
        output['instr_id'] = inputs['instr_id']
