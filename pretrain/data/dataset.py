...
TRAIN_MAX_STEP = 30
cands_pos = [0.7854, 0.8444, 0.895, 0.9328, 0.9756, 0.9952, 1.0]

class ReverieTextPathData(object):
    def __init__(
        self, anno_files, img_ft_files, obj_ft_file, scanvp_cands_file, connectivity_dir,
        image_feat_size=2048, image_prob_size=1000, angle_feat_size=4,
        obj_feat_size=None, obj_prob_size=None, max_objects=20,
        max_txt_len=100, in_memory=True, act_visited_node=False, sel_data_idxs=None
    ):
        self.img_ft_files = img_ft_files
    ...
    def get_scanvp_feature(self, scan, viewpoint):
        ...
        if self.in_memory and key in self._feature_store:
            ...
            with h5py.File(self.img_ft_files[0], 'r') as f:
                view_fts = f[key][...].astype(np.float32)
                if len(self.img_ft_files) > 1:
                    with h5py.File(self.img_ft_files[1], 'r') as f:
                        dino_ft = f[key][...].astype(np.float32)
                        view_fts = np.concatenate((view_fts, dino_ft), axis=1)
            obj_attrs = {}
    ...
    def get_act_labels(self, end_vp, end_idx, item, gmap_vpids, gmap_visited_masks, traj_cand_vpids):
        if end_vp == item['path'][-1]:  # stop
            global_act_label = local_act_label = 0
        else:
            global_act_label = local_act_label = -100
            # global: unvisited vp
            # use item['path'][ :end_idx + 1] to make masks
            masks = set()
            for vp in enumerate(item['path'][ :end_idx + 1]):
                masks.add(vp)
            # use item['path'][end_idx + 1: ] to choose gt_next_vp
            gt_next_vp = 0
            for vp in item['path'][end_idx + 1: ]:
                if vp not in masks:
                    gt_next_vp = vp; break
            if gt_next_vp == 0:
                global_act_label = local_act_label = 0
                ...
                for k, cand_vp in enumerate(gmap_vpids):
                    if (k > 0) and (not gmap_visited_masks[k]):
                        if cand_vp == gt_next_vp:
                            global_act_label = k; break # [stop] is 0
                # local: 
                cand_min_dist = float('inf')
                scan = item['scan']; pos_vps = [item['path'][-1]]
    ...
    def get_input(
        self, idx, end_vp_ratio, return_img_probs=False, return_act_label=False, 
        ...
        #         end_vp = end_vps[np.random.randint(len(end_vps))]
        end_idx =  len(gt_path) - 1 if isinstance(end_vp_ratio, str) else int(end_vp_ratio * len(gt_path))
        if end_idx >= len(gt_path):
            end_idx = len(gt_path) - 1
        end_vp = gt_path[end_idx]

        # gt_path = self.shortest_paths[scan][start_vp][end_vp]
        gt_path = gt_path[0: end_idx + 1]
        ...
        traj_view_img_fts, traj_obj_img_fts, traj_loc_fts, traj_nav_types, traj_cand_vpids, \
            last_vp_angles, last_vp_objids = self.get_traj_pano_fts(scan, gt_path, item)     
        ...
        global_act_label, local_act_label = self.get_act_labels(
            end_vp, end_idx, item, gmap_vpids, gmap_visited_masks, traj_cand_vpids
        )
        ...
        outs = {
            ...
            'global_act_labels': global_act_label,
            'local_act_labels': local_act_label,
        }
    ...
    def get_traj_pano_fts(self, scan, path, item):
        '''
        ...
        for vp in path:
            ...
            for k, v in nav_cands.items():
                ...
                viewIndex = item[vp] if vp in item else 12
                # TODO: whether using correct heading at each step
                view_angle = self.all_point_rel_angles[viewIndex][v[0]]
...
class GoalPathData(object):
    def __init__(
        ...
        self.img_ft_files = img_ft_files
        self.obj_ft_file = None#obj_ft_file
    ...
    def get_endvp(self, end_vp_ratio, gt_path):
        if isinstance(end_vp_ratio, str) or end_vp_ratio >= 0.9:
            return gt_path[-1]
        else:
            end_vp_ratio = end_vp_ratio/0.9
            end_idx = int(end_vp_ratio * len(gt_path))
            return gt_path[end_idx - 1] if end_idx >= len(gt_path) else gt_path[end_idx]
        # elif end_vp_ratio <= 0.75:
        #     end_idx = int(end_vp_ratio * len(gt_path))
        #     return gt_path[end_idx - 1] if end_idx >= len(gt_path) else gt_path[end_idx]
        # else:
        #     gt_vpset = set(gt_path)
        #     for i in range(len(gt_path)):
        #         if end_vp_ratio <= cands_pos[i] or i == len(gt_path):
        #             cand_vpids = list(self.scanvp_cands['%s_%s'%(scan, gt_path[i])].keys())
        #             end_vps = [cand for cand in cand_vpids if cand not in gt_vpset]
        #             return gt_path[i] if len(end_vps) == 0 else end_vps[np.random.randint(len(end_vps))]
        # return gt_path[-1]
    ...
    def get_image_feature(self, scan, viewpoint, target):
        key = '%s_%s' % (scan, viewpoint)
        if key in self._feature_store:
            ft, _, _ = self._feature_store[key]
        else:
            if ".hdf5" in self.img_ft_files[0]:
                with h5py.File(self.img_ft_files[0], 'r') as f:
                    ft = f[key][...].astype(np.float32)
                if len(self.img_ft_files) > 1:
                    with h5py.File(self.img_ft_files[1], 'r') as f:
                        dino_ft = f[key][...].astype(np.float32)
                    ft = np.concatenate((ft, dino_ft), axis=1)
            else:
                pass
    ...
    def get_input(
        ...
        target = item['target']
        instr_id = '%s_%s'%(path_id, target)

        # end_vp = self.get_endvp(end_vp_ratio, gt_path, scan)
        end_idx = len(gt_path) - 1 if isinstance(end_vp_ratio, str) else int(end_vp_ratio * len(gt_path))
        end_vp =  gt_path[end_idx - 1] if end_idx >= len(gt_path) else gt_path[end_idx]

        if 'instr_encoding' in item:
            img_embeds = None
            instr_encoding = item['instr_encoding'][:self.max_txt_len]
        else:
            img_embeds = self.get_image_feature(scan, end_vp, target)
            instr_encoding = None
    ...
    def get_cur_angle(self, scan, path, start_heading):
        if len(path) < 2:
            ...
            prev_vp = path[-2]
            cur_vp = path[-1]
            if not cur_vp in self.scanvp_cands['%s_%s'%(scan, prev_vp)]:
                print(scan, path)
            viewidx = self.scanvp_cands['%s_%s'%(scan, prev_vp)][cur_vp][0]
            heading = (viewidx % 12) * math.radians(30)
            elevation = (viewidx // 12 - 1) * math.radians(30)
    ...
    def get_traj_pano_fts(self, scan, path):
        ...
        '''
        traj_view_img_fts, traj_obj_img_fts, traj_loc_fts, traj_nav_types, traj_cand_vpids = [], [], [], [], []
        for index, vp in enumerate(path):
            view_fts, obj_img_fts, obj_attrs = self.get_scanvp_feature(scan, vp)

            view_img_fts, view_angles, cand_vpids = [], [], []
            nav_cands = self.scanvp_cands['%s_%s'%(scan, vp)]
            used_viewidxs = set()
            vidx = 12 if (index == 0 or index == TRAIN_MAX_STEP) else self.scanvp_cands['%s_%s'%(scan, path[index - 1])][vp][0]
            # k,v is like 97e40a...508284 [15, 0.17268776356842544, -0.1724669184212676, 0.008730735165851452]
            for k, v in nav_cands.items():
                used_viewidxs.add(v[0])
                view_img_fts.append(view_fts[v[0]])
                # TODO: whether using correct heading at each step
                view_angle = self.all_point_rel_angles[vidx][v[0]]
                # In order to get to the angle corresponding to v[0], how much should be transferred from viewIndex
                # v[0] is already gt, so just control viewIndex correctly.
                view_angles.append([view_angle[0] + v[2], view_angle[1] + v[3]])
                cand_vpids.append(k)
            view_img_fts.extend([view_fts[idx] for idx in range(36) if idx not in used_viewidxs])
            view_angles.extend([self.all_point_rel_angles[12][idx] for idx in range(36) if idx not in used_viewidxs])
