...
class GMapObjectNavAgent(Seq2SeqAgent):
    ...
    def _panorama_feature_variable(self, obs):
        ...
        for i, ob in enumerate(obs):
            ...
            # view_img_fts view_ang_fts and nav_types their length are all 36 candidate on the left others on the right
    ...
    def _nav_gmap_variable(self, obs, gmaps):
        ...
        for i, gmap in enumerate(gmaps):
            ...
            #record dists between all pairs in the form of matrix
            gmap_pair_dists = np.zeros((len(gmap_vpids), len(gmap_vpids)), dtype=np.float32)#matrix of dists
        ...
        #pad the matrix of dists to the size of the longest gmap
    ...
    def _image_target_variable(self, obs):
        mask = torch.ones((len(obs),1)).bool().cuda()
        features = np.vstack([ob['target_feature'] for ob in obs])
        features = torch.from_numpy(features).unsqueeze(-2).cuda()
        return {
            'target_features': features,
            'txt_masks': mask
        }

    def _noobj_nav_vp_variable(self, obs, gmaps, pano_embeds, cand_vpids, view_lens, nav_types):
        batch_size = len(obs)
        ...
        batch_vp_pos_fts = []
        for i, gmap in enumerate(gmaps):
            cur_cand_pos_fts = gmap.get_pos_fts(# pos from cand_vp
                obs[i]['viewpoint'], cand_vpids[i], 
                obs[i]['heading'], obs[i]['elevation']
            )
            cur_start_pos_fts = gmap.get_pos_fts(# pos from start_vp
                obs[i]['viewpoint'], [gmap.start_vp], 
                obs[i]['heading'], obs[i]['elevation']
            )                    
            # add [stop] token at beginning
    ...
    def _noobj_panorama_feature_variable(self, obs):
        ''' Extract precomputed features into variable. '''
    ...
    def _teacher_action_short(self, obs, vpids, ended, visited_masks=None):
        """
    ...
    def _teacher_action_self(self, obs, vpids, ended, traj, visited_masks=None):# used in learning self exploration path in img goal
        """
        Extract teacher actions into variable.
        :param obs: The observation.
        :param ended: Whether the action seq is ended
        ...
        for i, ob in enumerate(obs):
            if ended[i]:                                            # Just ignore this index
                ...
                if ob['viewpoint'] == ob['gt_path'][-1]:
                    ...
                    scan = ob['scan']
                    min_idx = self.args.ignoreid
                    gt_next_vp = 0
                    for index in range(len(sum(traj[i]['path'], [])), len(obs[i]['gt_path'])):
                        if not obs[i]['gt_path'][index] in traj[i]['path_set']:
                            gt_next_vp = obs[i]['gt_path'][index]; break
                    if gt_next_vp == 0:
                        a[i] = 0
                    else:
                        for j, vpid in enumerate(vpids[i]):
    ...
    def make_equiv_action(self, a_t, gmaps, obs, traj=None):
        ...
        for i, ob in enumerate(obs):
            ...
            if action is not None:            # None is the <stop> action
                ...
                for vp in gmaps[i].graph.path(ob['viewpoint'], action):
                    traj[i]['path_set'].add(vp)
    ...
    def _update_scanvp_cands(self, obs):
        for ob in obs:
            ...
            #dict_keys(['heading', 'elevation', 'normalized_heading', 'normalized_elevation', 'scanId', 'viewpointId', 'pointId', 'distance', 'idx', 'feature', 'position'])
                self.scanvp_cands[scanvp].setdefault(cand['viewpointId'], {})
                self.scanvp_cands[scanvp][cand['viewpointId']] = cand['pointId']# [0,36]
    ...
    def rollout(self, train_ml=None, train_rl=False, reset=True, og_loss_weight=1):
        ...
        use_obj = ('target' not in obs[0])

        traj = [{
            'instr_id': '%s_%s'%(ob['instr_id'],ob['scan']),
            'path': [[ob['viewpoint']]],
            'target': None if use_obj else ob['target'],
            'pred_objid': None,
            'details': {},
            'path_set': {ob['viewpoint']} # used in learning self exploration path in img goal
        } for ob in obs]
        
        # Language input: txt_ids, txt_masks
       
        if not obs[0]['instr_encoding'] is None:
            language_inputs = self._language_variable(obs)
            txt_embeds = self.vln_bert('language', language_inputs)
        else:
            language_inputs = self._image_target_variable(obs)
            txt_embeds = self.vln_bert('image', language_inputs)
        
        # end_embeds = txt_embeds.cpu().detach().numpy()
        # for i, traj_item in enumerate(traj):
        #     traj_item['embed'] = end_embeds[i].squeeze()
        ...
        if use_obj:
            for t in range(self.args.max_action_len):
                ...
                if train_ml is not None:
                    # Supervised training
                    nav_targets = self._teacher_action_short(
                        obs, nav_vpids, ended, 
                    ...
                    if self.args.fusion in ['avg', 'dynamic'] and self.args.loss_nav_3:
                        ...
                        local_nav_targets = self._teacher_action_short(
            ...
            for t in range(self.args.max_action_len):
                for i, gmap in enumerate(gmaps):#len is batch_size
                    if not ended[i]:
                        gmap.node_step_ids[obs[i]['viewpoint']] = t + 1
                # graph representation
                pano_inputs = self._noobj_panorama_feature_variable(obs)
                pano_embeds, pano_masks = self.vln_bert('panorama', pano_inputs)
                avg_pano_embeds = torch.sum(pano_embeds * pano_masks.unsqueeze(2), 1) / \
                                torch.sum(pano_masks, 1, keepdim=True)
                ...
                for i, gmap in enumerate(gmaps):
                    if not ended[i]:
                        # update visited node: current node
                        i_vp = obs[i]['viewpoint']
                        gmap.update_node_embed(i_vp, avg_pano_embeds[i], rewrite=True)
                        # update unvisited nodes: current node's candidates
                        for j, i_cand_vp in enumerate(pano_inputs['cand_vpids'][i]):
                            if not gmap.graph.visited(i_cand_vp):
                                gmap.update_node_embed(i_cand_vp, pano_embeds[i, j])
                # navigation policy
                ...
                # update graph, update the nav_probs to the viewpoint
                for i, gmap in enumerate(gmaps):
                    if not ended[i]:
                        i_vp = obs[i]['viewpoint']
                        gmap.node_stop_scores[i_vp] = {
                ...
                if train_ml is not None:
                    ...
                    if self.feedback == 'sample':
                        ...
                        nav_targets = self._teacher_action_self(
                            obs, nav_vpids, ended, traj,
                            visited_masks=nav_inputs['gmap_visited_masks'] if self.args.fusion != 'local' else None
                        )
                    ml_loss += self.criterion(nav_logits, nav_targets)
                ...
                if self.feedback == 'teacher':
                    ...
                    c = torch.distributions.Categorical(nav_probs)
                    self.logs['entropy'].append(c.entropy().sum().item())            # For log the value of entropy
                    entropys.append(c.entropy())                                     # For optimization
                    a_t = c.sample().detach() 
                elif self.feedback == 'expl_sample':
                    _, a_t = nav_probs.max(1)
                ...
                # Prepare environment action
                cpu_a_t = []# action for the whole batch
                for i in range(batch_size):
                    if a_t_stop[i] or ended[i] or nav_inputs['no_vp_left'][i] or (t == self.args.max_action_len - 1):
                        cpu_a_t.append(None)
                        just_ended[i] = True
