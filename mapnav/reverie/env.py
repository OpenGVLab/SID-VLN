...
class ReverieObjectNavBatch(object):
    ...
    def _get_gt_trajs(self, data):
        gt_trajs = {
            '%s_%s'%(x['instr_id'], x['scan']): (x['scan'], x['path'], x['objId']) \
    ...
    def _eval_item(self, scan, pred_path, pred_objid, gt_path, gt_objid):
        ...
        shortest_distances = self.shortest_distances[scan]#matrix in the form of double dict
...
class ImgNavBatch(object):
    ''' Implements the Image navigation task, using discretized viewpoints and pretrained features '''

    def __init__(
        self, view_db, data_path, connectivity_dir,
        batch_size=64, angle_feat_size=4, max_objects=None, seed=0, name=None, sel_data_idxs=None,
        num_mp3d_scans=None, scan=None
    ):
        self.env = EnvBatch(connectivity_dir, feat_db=view_db, batch_size=batch_size)

        if not scan is None:
            self.output_dir = f'your_path/datasets/REVERIE/annotations/scale_round1_hm3d/{int(scan)}.jsonl'
        ...
        self.sim = new_simulator(self.connectivity_dir)
        self.angle_feature = get_all_point_angle_feature(self.sim, self.angle_feat_size)#[36,36,4]
        #sin(heading) cos(heading) sin(elevation) cos(elevation)
        self.buffered_state_dict = {}
        print('%s loaded with %d data, using splits: %s' % (
            self.__class__.__name__, len(self.data), self.name))

    def _get_gt_trajs(self, data):
        gt_trajs = {
            '%s_%s_%s'%(x['path_id'],x['target'],x['scan']): (x['scan'], x['path']) \
                for x in data 
        }
        return gt_trajs
    ...
    def _next_minibatch(self, batch_size=None, **kwargs):
        ...
        self.batch = batch
        if self.ix % 32768 == 0:
            print('self.ix:',self.ix, end=" ")
        # random sample target img at each minibatch:
        for b in self.batch:
            if not 'target' in b:
                b['target'] = sum(ord(char) for char in b['path_id'])%36 if self.eval else np.random.randint(0,35)
            b['instr_id'] = '%s_%s'%(b['path_id'],b['target'])
            b['target_feature'] = self.env.feat_db.get_image_feature(b['scan'],b['path'][-1])[b['target']]
    ...
    def _get_obs(self):
        ...
        for i, (feature, state) in enumerate(self.env.getStates()):
            ...
            ob = {
                ...
                'candidate': candidate,
                'navigableLocations' : state.navigableLocations,
                'instr_encoding': item['instr_encoding'] if 'instr_encoding' in item else None,
                'gt_path' : item['path'],
                'target' : item['target'],
                'path_id' : item['path_id']
            }
            ob['target_feature'] = item['target_feature']
            obs.append(ob)
    ...
    ############### Nav Evaluation ###############
    def _eval_item(self, scan, pred_path, gt_path, embed=None, gt_objid=None):#finish
        scores = {}

        shortest_distances = self.shortest_distances[scan]#matrix dict:{key:dict, key:dict...}
        ...
        scores['action_steps'] = len(pred_path) - 1
        scores['trajectory_steps'] = len(path) - 1
        scores['trajectory_lengths'] = np.sum([shortest_distances[a][b] for a, b in zip(path[:-1], path[1:])])#calculate the whole lengths by summing the distances
        gt_lengths = np.sum([shortest_distances[a][b] for a, b in zip(gt_path[:-1], gt_path[1:])])
        # end_embeds = self.env.feat_db.get_image_feature(scan, path[-1])
        ...
        # scores['cos_similarity'] = cosine_similarity(embed, end_embeds)
        dis=shortest_distances[gt_path[-1]][path[-1]]
        scores['3m_success']= float(dis<=3.0)
        scores['success'] = float(path[-1] in goal_viewpoints)
        scores['oracle_success'] = float(any(x in goal_viewpoints for x in path))
        scores['spl'] = scores['success'] * gt_lengths / max(scores['trajectory_lengths'], gt_lengths, 0.01)
    ...
    def eval_metrics(self, preds):#finish
        ''' Evaluate each agent trajectory based on how close it got to the goal location 
        the path contains [view_id, angle, vofv]'''
        print('eval %d predictions' % (len(preds)))
        metrics = defaultdict(list)
        ...
        avg_metrics = {
            ...
            'steps': np.mean(metrics['trajectory_steps']),
            'lengths': np.mean(metrics['trajectory_lengths']),
            '3m_sr': np.mean(metrics['3m_success']) * 100,
            'sr': np.mean(metrics['success']) * 100,
            'oracle_sr': np.mean(metrics['oracle_success']) * 100,
            'spl': np.mean(metrics['spl']) * 100,
            # 'similarity': np.max(metrics['cos_similarity'])
        }
        return avg_metrics, metrics

    def extract_data(self, preds):#finish
        ''' Evaluate each agent trajectory based on how close it got to the goal location 
        the path contains [view_id, angle, vofv]'''
        print('eval %d predictions' % (len(preds)))
        metrics = defaultdict(list)
        for item in preds:
            ...
            scan, gt_traj = self.gt_trajs[instr_id]
            path = sum(traj, [])
            assert gt_traj[0] == path[0]
            if gt_traj[-1] == path[-1]:
                new_item={}
                new_item['instr_id'] = item['instr_id']
                # new_item['gt_path'] = self.gt_trajs[instr_id]
                new_item['path_id']=instr_id.split('_')[0] + '_' + instr_id.split('_')[1]
                new_item['path']=path
                new_item['target']=item['target']
                new_item['heading']=0.0
                new_item['scan']=scan
                open(f'your_path/hm3d_{self.name}.jsonl', 'a', encoding='utf-8').write(json.dumps(new_item, ensure_ascii=False) + '\n')
