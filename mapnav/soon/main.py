...
from soon.env import SoonObjectNavBatch, ImgNavBatch
...
def build_dataset(args, rank=0, is_test=True):
    ...
    train_env = dataset_class(
        ...
        sel_data_idxs=None if args.world_size < 2 else (rank, args.world_size), name='train',  is_train=True,
...
def zero_shot(args, val_envs, rank):
    ...
    agent_class = SoonGMapObjectNavAgent
    ckpt_dir = "your_path/exprs_map/pretrain/xxx/ckpts"
    best_val = {
        'val_unseen_instrs': {'spl': 0., 'sr': 0., 'state': ''},
        'val_unseen_house': {'spl': 0., 'sr': 0., 'state': ''},
    }    
