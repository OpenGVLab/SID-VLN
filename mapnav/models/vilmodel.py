...
class GlocalTextPathNavCMT(BertPreTrainedModel):
    ...
    def forward_image(self, target_features):
        img_query_embeddings = self.img_query_embeddings(target_features)

        return img_query_embeddings
    ...
    def forward_navigation_per_step(
        ...
        gmap_embeds = self.global_encoder.encoder(
            txt_embeds, txt_masks, gmap_embeds, gmap_masks,
            graph_sprels=graph_sprels#it is from dists
        )
        ...
        fused_logits[:, 0] += local_logits[:, 0]   # stop, the first element is stop
        for i in range(batch_size):
    ...
    def forward(self, mode, batch, **kwargs):
        if mode == 'language':
            return self.forward_text(batch['txt_ids'], batch['txt_masks'])
        if mode == 'image':
            return self.forward_image(batch['target_features'])
        elif mode == 'panorama':
